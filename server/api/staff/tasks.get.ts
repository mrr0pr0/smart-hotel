  ...
  try {
    ...
    const tasksWithUsers = await Promise.all(
      allTasks.map(async (task) => {
        const user = await db.select().from(users).where(eq(users.id, task.assignedTo)).limit(1)
        return {
          ...task,
          assignedUser: user[0] ? { id: user[0].id, name: user[0].name, role: user[0].role } : null
        }
      })
    )
